...
class GetCareerPath extends Component {
  ...
  loadData() {
    ...
    AuthenticationDataService.getPlusTwo().then((response) => {
  ...
  onSubmit(values) {
    ...
    AuthenticationDataService.studentCareerPath(requestBody).then(
      (response) => {
  ...
  loadGraduation(event) {
    ...
    AuthenticationDataService.getGraduation(event.target.value).then(
      (response) => {
  ...
  loadPg(event) {
    ...
    AuthenticationDataService.getPostGraduation(
  ...
  loadRoute(event) {
    ...
    AuthenticationDataService.getRole(
      this.state.ptselected,
      this.state.gselected,
      event.target.value
    ).then((response) => {
              ...
              <form>
                <div className="form-group">
                  ...
                  <select
                    class="form-control"
                    name="ptselected"
                    onChange={this.loadGraduation}
                    value={this.state.ptselected}
                  >
                    <option key="select">select</option>
                    {this.state.plustwo.map((pt) => (
                ...
                <div className="form-group">
                  ...
                  <select
                    ...
                    onChange={this.loadPg}
                    value={this.state.gselected}
                  >
                    <option key="select">select</option>
                    {this.state.graduation.map((gd) => (
                      <option key={gd}>{gd}</option>
                    ))}
                  </select>
                </div>
                ...
                <div className="form-group">
                  ...
                  <select
                    class="form-control"
                    name="plustwovalue"
                    onChange={this.loadRoute}
                    value={this.state.pgselected}
                  >
                    <option key="select">select</option>
                    {this.state.pg.map((pt) => (
                      <option key={pt}>{pt}</option>
                    ))}
                  </select>
                </div>
                ...
                <div className="form-group">
                  <label for="exampleFormControlSelect1">Career Path</label>
                  <select
                    class="form-control"
                    name="plustwovalue"
                    onChange={this.saveCareerPath}
                  >
                    <option key="select">select</option>
                    {this.state.career.map((pt) => (
                      <option key={pt}>{pt}</option>
                    ))}
                  </select>
                </div>
